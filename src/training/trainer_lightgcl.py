...
def compute_metrics(predictions: Dict[int, np.ndarray],
    ...
    for user in test_data:
        ...
        # Get scores and mask training items
...
class LightGCLTrainer:
    ...
    def create_adj_matrix(self, train_data: List[Tuple[int, int]]) -> torch.sparse.Tensor:
        """
        Create normalized adjacency matrix from training data
        
        Args:
            train_data: List of (user_idx, item_idx) tuples
            
        Returns:
            Normalized adjacency matrix (sparse tensor)
        """
        print("\nCreating adjacency matrix...")
        
        # Create sparse interaction matrix R
        ...
        # Compute SVD for contrastive learning
    ...
    def evaluate(self, test_data: Dict[int, List[int]],
        ...
        with torch.no_grad():
            user_emb, item_emb = self.model.forward(self.adj_norm)
        ...
        # Compute metrics
        metrics = compute_metrics(predictions, test_data, train_dict, k_list)
