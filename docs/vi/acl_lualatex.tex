...
\begin{abstract}
    \noindent
    Gợi ý tin tức tiếng Việt đặt ra những thách thức đặc thù do sự phức tạp của ngôn ngữ, tương tác người dùng thưa thớt và vấn đề "Cold-Start". Trong bài báo này, chúng tôi đề xuất một \textbf{phân tích chi tiết và toàn diện về} quy trình gợi ý, tận dụng Multi-Aspect Heterogeneous Graph Neural Networks (MA-HGN) và Contrastive Learning (CL). Chúng tôi giới thiệu một phương pháp xây dựng Heterogeneous Graph mới kết hợp người dùng, bài viết, danh mục và tác giả. Chúng tôi đánh giá so sánh các mô hình Collaborative Filtering (CF) tiên tiến, bao gồm LightGCL, SimGCL, và XSimGCL, cùng với các triển khai tùy chỉnh MA-HCL và HGN của chúng tôi. Các thực nghiệm \textbf{trên diện rộng} với bộ dữ liệu VnExpress thực tế gồm 15.693 người dùng và 3.645 bài viết chứng minh rằng: trong khi các phương pháp Content-Based (PhoBERT) vẫn dẫn đầu về truy xuất toàn cục (Recall@10 đạt 0.0535), mô hình HGN (G2) của chúng tôi đạt độ ổn định vượt trội trong xếp hạng Leave-One-Out (Recall@10 đạt 0.4364), nhờ nắm bắt hiệu quả các tín hiệu social homophily.
\end{abstract}
...
Với dữ liệu tin tức, tương tác có tính thời gian mạnh. Khi xây dựng tập train/test, việc đảm bảo train thuộc quá khứ và test thuộc tương lai giúp mô phỏng đúng bối cảnh triển khai \cite{wang2023leakage}.
...
\subsection{Kết luận}
Báo cáo này trình bày một nghiên cứu toàn diện về hệ thống gợi ý bài báo tiếng Việt dựa trên học đồ thị dị thể (Heterogeneous Graph Learning), với mục tiêu giải quyết các thách thức cố hữu của bài toán recommendation trong lĩnh vực tin tức, bao gồm: dữ liệu thưa thớt, vòng đời nội dung ngắn, và sự thiếu hụt tương tác trực tiếp của người dùng. Dựa trên dữ liệu thực tế từ VnExpress với hơn 15.000 người dùng và 3.600 bài viết, nhóm đã xây dựng một pipeline thu thập – xử lý – chuẩn hóa dữ liệu hoàn chỉnh, kèm theo các phân tích khám phá (EDA) nhằm nhận diện đặc trưng thống kê, phân bố và mức độ thưa của tập dữ liệu.

Về mặt mô hình, nhóm đề xuất một hướng tiếp cận theo tầng (G1–G3), mở rộng dần cấu trúc đồ thị từ dạng hai phần (bipartite) truyền thống sang đồ thị xã hội (user–user) và đồ thị có trung tâm chuyên mục (category hubs). Dựa trên nền tảng đó, mô hình Heterogeneous Graph Network (HGN) được thiết kế để khai thác đồng thời hai loại tín hiệu quan trọng: (i) sở thích cá nhân thể hiện qua tương tác user–article, và (ii) ảnh hưởng xã hội phản ánh qua quan hệ reply giữa người dùng. Ngoài ra, nhóm cũng thử nghiệm mô hình Heterogeneous Contrastive Learning (HCL) để đánh giá khả năng tăng độ ổn định của embedding trong môi trường dữ liệu cực thưa.
