...
\begin{abstract}
News recommendation in Vietnamese presents unique challenges due to linguistic complexity, sparse user interactions, and the "cold-start" problem. In this work, we propose a comprehensive breakdown of the recommendation pipeline, leveraging Multi-Aspect Heterogeneous Graph Neural Networks (MA-HGN) and Contrastive Learning (CL). We introduce a novel heterogeneous graph construction (G5) incorporating users, articles, categories, and authors, enriched with weighted edges for engagement and recency. We benchmark state-of-the-art Collaborative Filtering (CF) models, including NGCF, LightGCL, SimGCL, and XSimGCL, alongside our custom implementations of MA-HCL and SimMAHGN. Our extensive experiments on a real-world VnExpress dataset utilizing 30k users and 12k articles demonstrate that our hybrid approach, combining SimGCL with VN-SBERT embeddings, achieves superior performance (NDCG@10 of 0.1895) compared to traditional methods. We also present a detailed ablation study on edge types and embedding models, highlighting the critical role of semantic enrichment in news recommendation.
\end{abstract}
...
\begin{enumerate}
    \item \textbf{Heterogeneous Graph Construction:} We construct extensive graph variants (G1-G5), integrating Author and Category nodes, and introducing weighted edges based on user engagement (comments, replies) and article freshness.
...
We crawled data from VnExpress, a major Vietnamese news portal, resulting in over 12,000 articles and 50,000 user interactions. The data includes:
\begin{itemize}
...
We experimented with five graph variants (G1-G5) to isolate the impact of different signals. The most comprehensive graph, \textbf{G5}, includes:
\begin{enumerate}
    \item \textbf{User-Article Edges ($E_{ui}$):} Weighted by interaction depth ($w=1$ for view, $w=2$ for comment, $w=3$ for reply).
...
\subsubsection{Impact of Edge Types (G1-G5)}
We analyzed the contribution of different edge types.
\begin{itemize}
    \item \textbf{G1 (User-Article Binary):} Baseline performance.
    \item \textbf{G2 (+ Weighted Interactions):} +5.2\% NDCG gain. Differentiating comments from views captures explicit user interest.
    \item \textbf{G5 (+ Author + Category Nodes):} +2.1\% additional gain. Semantic connections via shared authors/topics help propagate signals across unconnected users.
\end{itemize}
